...
def add_task():
    ...
    if len(title) < 3:
        flash("Task title must contain at least 3 characters.", "error")
        return redirect("/dashboard")
    ...
    if len(description) < 5:
        flash("Description must contain at least 5 characters.", "error")
        return redirect("/dashboard")


    # Validate priority
    if not priority:
        flash("Please select a priority.", "error")
        return redirect("/dashboard")
